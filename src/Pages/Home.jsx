...
import { CartArray } from '../store/CartSlice'
...
const Home = () => {
  ...
  return (
    ...
    {
      products.map((ele,i)=>{
        return   <div class="bg-gray-200 flex flex-col rounded-md cursor-pointer transition-all relative overflow-hidden">
        ...
        <div class="flex flex-col h-full text-center bg-gray-100 p-4">
          <div class="flex-1">
            <h3 class="text-sm sm:text-base font-bold text-gray-800">{ele.title}</h3>
            <h4 class="text-sm sm:text-base text-gray-800 font-bold mt-4">{ele.price} <strike
              class="text-gray-400 ml-2 font-medium">$15</strike>
            </h4>
          </div>

          <button type="button" onClick={()=>dispatch(CartArray(ele))}
